...
tracegen -otlp-endpoint localhost:55685 -otlp-insecure -service e2e-test-agent-without-auth
if [ $? != 0 ]; then
    echo "Failed to generate a trace to the agent that does not authenticate with the collector."
    exit 1
fi

tracegen -otlp-endpoint localhost:55680 -otlp-insecure -service e2e-test-agent-with-auth
if [ $? != 0 ]; then
